    ...
    <div className="min-h-screen" style={{ backgroundColor: '#FCFAF5' }}>
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm" style={{ backgroundColor: 'rgba(74, 165, 255, 0.5)' }}>
            ...
            <nav className="flex flex-col items-center gap-8">
              <button onClick={() => { onNavigate('home'); setIsMenuOpen(false); }} className="text-6xl text-white transition-colors hover:opacity-80" style={{ fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>Home</button>
              <button onClick={() => { onNavigate('events'); setIsMenuOpen(false); }} className="text-6xl text-white transition-colors hover:opacity-80" style={{ fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>Events</button>
              <button onClick={() => { onNavigate('spots'); setIsMenuOpen(false); }} className="text-6xl text-white transition-colors hover:opacity-80" style={{ fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>Best Spots</button>
              <button onClick={() => setIsMenuOpen(false)} className="text-6xl transition-colors hover:opacity-80" style={{ color: '#FBED4F', fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>Map</button>
              <button onClick={() => { onNavigate('contact'); setIsMenuOpen(false); }} className="text-6xl text-white transition-colors hover:opacity-80" style={{ fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>Contact</button>
            </nav>
          </div>
        )}
      ...
      <div className="container mx-auto px-8 pt-8 flex flex-wrap justify-between items-center gap-4">
        <div className="flex gap-2 flex-wrap">
          {types.map((type) => (
            <button
              ...
              className={`px-6 py-3 rounded-full transition-all shadow-lg ${selectedType === type ? 'text-white' : 'text-gray-700 bg-white'}`}
              style={{
                backgroundColor: selectedType === type ? '#4AA5FF' : undefined
              }}
            >
        ...
        <div className="flex gap-2">
          ...
          <button className="p-3 text-white rounded-full" style={{ backgroundColor: '#4AA5FF' }}>
      ...
      {loading && (
        <div className="container mx-auto px-8 py-8 text-center">
          <p className="text-xl text-gray-600">Se încarcă locațiile...</p>
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="container mx-auto px-8 py-8">
          <div className="relative w-full h-[600px] bg-white rounded-3xl shadow-2xl overflow-hidden border-4" style={{ borderColor: '#4AA5FF' }}>
            ...
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 text-white px-6 py-3 rounded-full shadow-lg z-10" style={{ backgroundColor: '#4AA5FF' }}>
          ...
          <div className="mt-6 bg-white rounded-3xl shadow-lg p-6 border-4" style={{ borderColor: '#5ECCAD' }}>
            <h3 className="text-3xl text-gray-800 mb-6" style={{ fontFamily: 'Retail Heavy, Montserrat, sans-serif', fontWeight: 900 }}>
      ...
      {/* Footer */}
      <footer className="relative py-16 overflow-hidden mt-12" style={{
